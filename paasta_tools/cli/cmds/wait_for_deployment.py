...
def get_instance_status(service, instance, cluster_url):
    url = '{cluster_url}/v1/{service}/{instance}/status'.format(
            cluster_url=cluster_url, service=service, instance=instance)
...
def wait_for_deployment(check_map, attempts=None, interval=5):
    ...
    for service, svc_data in check_map.items():
        clusters = svc_data['clusters']
        instances = svc_data['instances']
        for cluster_url in clusters:
            cluster_name = urlparse.urlparse(cluster_url).hostname
            cluster_map[cluster_name] = cluster_url
            for instance in instances:
                i_name = '{0}|{1}|{2}'.format(cluster_name, service, instance)
                to_check.append(i_name)
    ...
    while True:
        ...
        for i_name in to_check:
            if i_name in failed:
                continue
            cluster_name, service, instance = i_name.split('|')
            print 'Checking {0}.{1} at {2}'.format(service, instance, cluster_name)
            cluster_url = cluster_map[cluster_name]
            i_data = get_instance_status(service, instance, cluster_url)
            if not i_data:
                print '\tCheck failed.'
...
def paasta_wait_for_deployment(args):
    ...
    cluster_list = ['{proto}://{cluster}:{port}'.format(proto=proto,
        cluster=fqdn_format.format(cluster=cluster),
        port=system_paasta_config.get_api_port()) for cluster in cluster_list]
