...
def is_safe_to_kill(hostname):
    ...
    """
    if is_host_drained(hostname) or hostname in get_hosts_in_maintenance_window():
        return True
    return False
...
def is_host_drained(hostname):
    ...
    if is_host_draining(hostname=hostname) and slave_task_count == 0:
        return True
    return False


def get_hosts_in_maintenance_window():
